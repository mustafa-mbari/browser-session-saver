...
export async function POST(request: Request) {
  try {
    ...
    if (!priceId || (typeof priceId === 'string' && priceId.startsWith('REPLACE_'))) {
      return NextResponse.json({ error: 'Stripe price not configured for this plan. Run migration 040 first.' }, { status: 500 })
    }
