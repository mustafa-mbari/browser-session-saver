...
import { quicklinkMapper } from '@core/services/sync/row-mappers/quicklink.mapper';
import {
...
describe('quicklinkMapper', () => {
  const userId = 'user-ql-1';
  const link = {
    id: 'ql-1',
    title: 'Google',
  ...
  it('toRow maps camelCase to snake_case', () => {
    const row = quicklinkMapper.toRow(link, userId);
    expect(row.id).toBe('ql-1');
    expect(row.user_id).toBe(userId);
    expect(row.title).toBe('Google');
    expect(row.url).toBe('https://google.com');
    expect(row.fav_icon_url).toBe('https://google.com/favicon.ico');
    expect(row.position).toBe(0);
    expect(row.is_auto_generated).toBe(false);
  });
  ...
  it('fromRow maps snake_case to camelCase', () => {
    const row = {
      ...
      title: 'Google',
      url: 'https://google.com',
      fav_icon_url: 'https://google.com/favicon.ico',
      position: 0,
      is_auto_generated: false,
    };
    const restored = quicklinkMapper.fromRow(row);
    expect(restored.id).toBe('ql-1');
    expect(restored.title).toBe('Google');
    expect(restored.url).toBe('https://google.com');
    expect(restored.favIconUrl).toBe('https://google.com/favicon.ico');
    expect(restored.position).toBe(0);
    expect(restored.isAutoGenerated).toBe(false);
  });

  it('round-trips entity → row → entity losslessly', () => {
    const row = quicklinkMapper.toRow(link, userId);
    const restored = quicklinkMapper.fromRow(row);
    expect(restored).toEqual(link);
  });

  it('fromRow applies defaults for missing optional fields', () => {
    const row = { id: 'ql-2', title: 'Test', url: 'https://example.com' };
    const restored = quicklinkMapper.fromRow(row);
    expect(restored.favIconUrl).toBe('');
    expect(restored.position).toBe(0);
    expect(restored.isAutoGenerated).toBe(false);
  });
});
