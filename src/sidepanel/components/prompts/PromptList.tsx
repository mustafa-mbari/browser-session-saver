...
export default function PromptList({
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-3 pt-2 pb-1 flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] text-[var(--color-text)] placeholder:text-[var(--color-text-secondary)] focus:outline-none focus:ring-1 focus:ring-amber-400 transition-colors"
          />
        </div>
        <button
      ...
      {/* Filter panel */}
      {showFilters && (
        <div className="px-3 pb-2 space-y-2 border-b border-[var(--color-border)]">
          {/* Category filter */}
          {categories.length > 0 && (
            <div>
              <p className="text-[10px] font-medium text-[var(--color-text-secondary)] mb-1">Category</p>
          ...
          {tags.length > 0 && (
            <div>
              <p className="text-[10px] font-medium text-[var(--color-text-secondary)] mb-1">Tags</p>
              <div className="flex flex-wrap gap-1">
                {tags.map((tag) => (
                  <button
                    key={tag.id}
          ...
          <div className="flex items-center flex-wrap gap-3">
            <label className="flex items-center gap-1.5 cursor-pointer">
              ...
              />
              <span className="text-[10px] text-[var(--color-text-secondary)]">Favorites only</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input
                type="checkbox"
                ...
                className="w-3 h-3 accent-amber-500"
              />
              <span className="text-[10px] text-[var(--color-text-secondary)]">Group by category</span>
            </label>
          </div>

          {/* Sort */}
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-[var(--color-text-secondary)]">Sort:</span>
          ...
          Array.from(grouped.entries()).map(([catId, items]) => {
            ...
            return (
              <div key={catId} className="mb-3">
                <p className="text-[10px] font-semibold text-[var(--color-text-secondary)] uppercase tracking-wide mb-1.5 flex items-center gap-1">
